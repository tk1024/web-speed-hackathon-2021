...
const UserProfileContainer = (props: Props) => {
  const { data: user, isLoading: isLoadingUser } = useFetch(`/api/v1/users/${props.username}`, fetchJSON);
  const { data: posts, fetchMore } = useInfiniteFetch(`/api/v1/users/${props.username}/posts`, fetchJSON);
